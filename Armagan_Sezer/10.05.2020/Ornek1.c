#include <stdio.h>
#include <string.h>
int menu, ID, kisibul(), nufus = 5,derssayisi=0,kisisayisi=0,createbook(),booklist(),deletebook(),del,createuser(),userlist(),deleteuser(),budgetmenuchoice,sel,studentmoneyadd,budgetmenu(),studentmoneysubtract,userbudgetmenu(),borrowbook(),showmybooks(),deletemybook(),showstudents(),i,j;
...
int createbook(){
 ...
 scanf("%s" , &bookcopy[derssayisi][0]);
  printf("%s - %s\n", &bookname[derssayisi][0], &bookcopy[derssayisi][0]);
 derssayisi++;
 return 0;
  }
int booklist(){
  for(int i=0; i<derssayisi; i++){
    printf("%d Book: %s (%s copies)\n",i+1, &bookname[i][0], &bookcopy[i][0]);
  }
  return 0;
  }
int deletebook(){
...
int userlist(){
  for(int i=0; i<kisisayisi; i++){
    printf("%d User: %s (password %s, budget %d)\n",i+1, &studentname[i][0], &studentpassword[i][0], studentbudget[i][0]);
  }
  return 0;
  }
int deleteuser(){
  ...
  for(int i=0; i<kisisayisi; i++){
    ...
    studentbudget[i][0] = studentbudget[i+1][0];
     studentbudget[i+1][0] = '\0';
  }
...
int budgetmenu(){
  ...
  switch (budgetmenuchoice){
    ...
    case 2: printf("How much book do you want to remove?\n");
...
int showmybooks(){
 ...
 for(int a=0;a<j;a++){
 printf("%d.  bookname:%s  copies:%s\n ",a+1,&bookname[a][0], &bookcopy[50+a][0]);
 }
 return 0;
 }
